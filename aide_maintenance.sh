...
send_email() {
    ...
    if [[ -n "$SMTP_SERVER" ]]; then
        if command -v curl >/dev/null 2>&1; then
            ...
            curl_cmd+=("--upload-file" "$temp_email")
    ...
    if [[ "$email_sent" == false ]]; then
        if command -v mail >/dev/null 2>&1; then
            ...
            /usr/sbin/sendmail -t <"$temp_email" && email_sent=true
        fi
    fi
...
if [[ -f "$OLD_DB" ]]; then
    ...
    if ! "${decompress_cmd[@]}" "$OLD_DB" | "${COMPRESS[@]}" >"$BACKUP_DB"; then
...
if [[ ! -f "$NEW_DB" ]]; then
    fail "New AIDE database not found at: $NEW_DB"
fi

# Verify the new database is not empty
if [[ ! -s "$NEW_DB" ]]; then
    fail "New AIDE database is empty: $NEW_DB"
fi
...
"
    ...
    while IFS= read -r line; do
        decompress_cmd+=("$line")
    done < <(determine_decompression "$COMPRESSED_LOG_FILE")
    
    log_content="$("${decompress_cmd[@]}" "$COMPRESSED_LOG_FILE" 2>/dev/null || echo "Failed to decompress log.")"
